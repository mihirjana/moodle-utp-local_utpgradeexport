...
*/
function local_utpgradeexport_write_csv_to_file($runhow, $data = null) {
  global $CFG, $DB;

  $config = get_config('local_utpgradeexport');
  ...
  if (empty($config->csvlocation)) {
      $config->csvlocation = $CFG->dataroot.'/utpgradeexport';
  }
  ...
  if ($fh = fopen($filename, 'w')) {
      ...
      fwrite($fh, implode(',', local_utpgradeexport_get_csv_headers())."\r\n");

      $rs = local_utpgradeexport_get_data($config->lastrun, $data);

      if ($rs->valid()) {

          $strnotattempted  = get_string('notattempted', 'local_utpgradeexport');
          $scalearray       = array();
...
*/
      ...
      if ($runhow == 'auto') set_config('local_utpgradeexport', 'lastrun', time());
 ...
 */
function local_utpgradeexport_get_data($from, $data = null) {
    global $DB;
 ...
 * @return array
 */
function local_utpgradeexport_get_csv_headers() {
    return array(
        get_string('idnumber',   'local_utpgradeexport'),
        get_string('unitcode',   'local_utpgradeexport'),
        get_string('batch',      'local_utpgradeexport'),
        get_string('results',    'local_utpgradeexport'),
        get_string('scale',      'local_utpgradeexport'),
        get_string('resultdate', 'local_utpgradeexport'),
        );
}
